...
llm-wiki 是小说资料的日常检索和定位入口。写作、查询、审查和连续性检查，应优先从本目录定位资料；单章龙骨通过 `wiki/outline.md` 链接到 `../小说工程/03_剧情/章节/` 后按需读取。

- `原始资料/` 保存不可改写的原始资源。
- `小说工程/` 保存正文草稿、章节成稿和写作过程文件。
...
所有长期使用的设定、人物、主线、时间线、伏笔和状态，都必须进入 `llm-wiki/`。如果 wiki 页面缺失或矛盾，再回查 `原始资料/` 或 `小说工程/`，并把有效结论补回 wiki。
...
- `../小说工程/03_剧情/章节/`：每章剧情龙骨，属于小说工程层，不纳入最终 wiki 图谱。
...
- `wiki/人物/`：人物卡。达到复用门槛的人物必须独立成页。
- `wiki/地点/`：地点卡。多章出现或带有空间规则的地点必须独立成页。
- `wiki/组织/`：组织、势力和群体卡。
- `wiki/规则/`：修行、符法、世界运行规则和术语说明。
- `wiki/物品/`：重要道具、证据、账册、符物和可复用物品。
- `wiki/场景/`：复杂或可复用重要场景卡。
- `wiki/伏笔/`：需要独立追踪的伏笔节点。
...
本文件是操作摘要。强制规则以根目录项目规则文件（`AGENTS.md` / `Claude.md`）和 `小说工程/03_剧情/章节/_chapter-spine-template.md` 的同步检查清单为准。
...
3. `小说工程/05_正文/` 中已经产生设定变化、人物变化、伏笔或关键事件的章节。
4. `小说工程/03_剧情/章节/` 中新增或变更的章节龙骨。

正文章节不建议全部长期摄入。只有当章节包含已确定的设定变化、人物变化、伏笔或关键事件时，才摄入或摘要后摄入。

外部资料和用户原始说明应先放入 `原始资料/`，再从中抽取条目进入 wiki。只有正文草稿、章节成稿或写作过程文件进入 `小说工程/`。
...
只有当 wiki 缺失、矛盾或需要证据时，才回查 `原始资料/` 或 `小说工程/`；回查后的有效结论必须补回 wiki。
...
8. 如正文或章节规划受影响，再更新 `小说工程/`。
9. 执行节点缺口检查：触发建卡门槛的内容若只存在于 `index.md` 摘要、分类主节点摘要、总账表或 `小说工程/` 模板中，不算同步完成。
...
- 不要让重要设定只停留在 `原始资料/` 或 `小说工程/`，必须整理进 wiki。
