...
| --- | --- | --- | --- | --- |
| pending | 原始资料/ | 原始资源层 | 用户资料、外部来源 | 按需导入 |
| pending | llm-wiki/wiki/outline.md | 剧情总纲 | 章节组功能、阶段推进、支线咬合、后续规划 | 按章节组维护 |
| pending | 小说工程/03_剧情/章节/ | 章节龙骨 | 章节推进、结尾变化、后续约束 | 按章维护 |
...
不要把临时讨论直接加入摄入清单。用户明确提供的原始资料先进入 `原始资料/`；需要长期检索、交叉引用和防矛盾的内容必须进入 `llm-wiki/`。只有正文草稿、章节成稿或写作过程文件写入 `小说工程/`。
