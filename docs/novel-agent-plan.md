...
- 写作前必须从当前小说项目派生临时内容执行标准，并用内容上下文胶囊隔离正文生成层，避免工程化流程污染小说正文。
...
- 内容上下文胶囊：只给正文生成层使用，包含已发生剧情、本章开场状态、出场人物状态、人物动机、本章核心冲突、本章必须推进、本章结尾变化和项目文风；不包含同步检查表、文件路径、发布记录、内部阶段编号、质量检测表、agent 操作说明或工程日志。
...
章节完成前还必须在正文初稿完成后执行原创性与文本质量检查。检查依据以根目录 `AIGC_DETECTION_PRINCIPLES.md`、`docs/aigc-quality-control.md` 和当前小说实例要求为准。
