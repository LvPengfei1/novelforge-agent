...
本目录用于按 llm-wiki 模式管理小说项目设定。优先使用框架的 `scripts/novelforge.py` 执行批准校验和章节封包；宿主无法运行脚本时，Codex 仍可按 `AGENTS.md`、Claude 可按 `Claude.md` 手工维护 wiki，但不得声称已经完成机器校验。

小说实例根目录的 `novelforge.json` 声明四份全局正本和禁止进入章节封包的路径。框架的 `scripts/novelforge.py` 只读取章节任务明确列出的文件，用于架构校验、来源锁定和写作/审稿输入隔离；它不自行检索整个 wiki。
...
- `NOVEL_REVIEW_PROTOCOL.md` 保存默认独立审稿协议，是正文审读的执行正本。
- `AIGC_DETECTION_PRINCIPLES.md` 保存检测方法与局限的研究资料，不默认进入审稿封包。
...
写正文前先确认项目简报、全书龙骨和总体大纲属于同一已批准架构，再由主智能体建立本章输入白名单与章节写作包。每章固定提供全局正本，只补入本章相关人物、关系、时间线、伏笔、地点、规则和必要的前章结尾。正式正文由不继承主对话的全新写作子智能体生成；它不得自行全库搜索。初稿完成后，再交给另一个全新审查子智能体整章阅读和比较近章结构。
...
11. 正文初稿完成后，依据 `NOVEL_REVIEW_PROTOCOL.md` 启动独立整章审稿；故事层不合格时重构场面，修改后重新完整阅读。
